...
<template>
    <div :data-open="open" :data-overflow-hidden="!!overflowHidden" :style="`--min-deployment-height: ${util.xIfTrueThenY(minDeploymentHeight, 0)}`" class="flex flex-col w-full border-b border-b-gray last:border-b-0   data-[overflow-hidden=true]:data-[open=true]:overflow-y-hidden">
        ...
        <Transition leave-from-class="opacity-100 max-h-96" leave-to-class="opacity-0 max-h-0 no-scrollbar" leave-active-class="transition-all no-scrollbar" enter-to-class="opacity-100 max-h-96" enter-active-class="transition-all no-scrollbar" enter-from-class="opacity-0 max-h-0 no-scrollbar">
            ...
            <div :data-min-deployment-height="!!minDeploymentHeight" class="flex flex-col overflow-y-auto h-full data-[min-deployment-height=true]:min-h-[var(--min-deployment-height)]" v-show="open">
